...
const confluenceClient_2 = require("./confluenceClient");
const path_1 = __importDefault(require("path"));
const confluenceFormatter_1 = require("./confluenceFormatter");
const confluenceValidator_1 = require("./confluenceValidator");
const confluenceSchema_1 = require("./confluenceSchema");
let outputChannel;
...
function activate(context) {
    ...
    function updateDiagnostics(document) {
        if (document.languageId === 'xml' || document.fileName.endsWith('.confluence')) {
            const diags1 = (0, confluenceValidator_1.getUnclosedOrUnopenedTagDiagnostics)(document.getText());
            const diags2 = (0, confluenceValidator_1.getConfluenceDiagnostics)(document.getText());
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(event => {
        if (event.document.languageId === 'xml' || event.document.fileName.endsWith('.confluence')) {
            const diags = (0, confluenceValidator_1.getUnclosedOrUnopenedTagDiagnostics)(event.document.getText());
    ...
    const getPageByTitleCmd = vscode.commands.registerCommand('confluence-smart-publisher.getPageByTitle', async (uri) => {
        if (!uri || !uri.fsPath) {
        ...
        try {
            await vscode.window.withProgress({
                ...
                const client = new confluenceClient_2.ConfluenceClient();
                const page = await client.getPageByTitle(spaceKey, title);
                if (!page) {
                    throw new Error('Página não encontrada.');
                }
                const pageId = page.id;
                const filePath = await client.downloadConfluencePage(pageId, confluenceClient_2.BodyFormat.STORAGE, uri.fsPath);
    ...
    const getPageByIdCmd = vscode.commands.registerCommand('confluence-smart-publisher.getPageById', async (uri) => {
        if (!uri || !uri.fsPath) {
            vscode.window.showErrorMessage('Selecione uma pasta para salvar a página.');
            return;
        }
    ...
    const createPageCmd = vscode.commands.registerCommand('confluence-smart-publisher.createPage', async (uri) => {
        if (!uri || !uri.fsPath) {
        ...
        try {
            await vscode.window.withProgress({
                ...
                const modeloPath = await client.downloadConfluencePage(modeloId, confluenceClient_2.BodyFormat.STORAGE, tempDir);
                const fs = await import('fs');
                let conteudo = fs.readFileSync(modeloPath, 'utf-8');
                conteudo = conteudo.replace(/<csp:file_id>.*?<\/csp:file_id>\s*/s, '');
                const novoArquivoPath = path_1.default.join(uri.fsPath, nomeArquivo.endsWith('.confluence') ? nomeArquivo : nomeArquivo + '.confluence');
    ...
    const formatConfluenceCmd = vscode.commands.registerCommand('confluence-smart-publisher.formatConfluence', async (uri) => {
        if (!uri || !uri.fsPath.endsWith('.confluence')) {
        ...
        try {
            ...
            const formatted = (0, confluenceFormatter_1.formatConfluenceDocument)(document.getText(), numberChapters, outputChannel);
    ...
    const confluenceFormatter = vscode.languages.registerDocumentFormattingEditProvider('confluence', {
        async provideDocumentFormattingEdits(document) {
            ...
            try {
                const config = vscode.workspace.getConfiguration('confluenceSmartPublisher');
                const numberChapters = config.get('format.numberChapters', false);
                formatted = (0, confluenceFormatter_1.formatConfluenceDocument)(text, numberChapters, outputChannel);
            }
            catch (e) {
                // Se der erro, retorna o texto original
            }
            return [
                vscode.TextEdit.replace(new vscode.Range(new vscode.Position(0, 0), new vscode.Position(document.lineCount, 0)), formatted)
            ];
        }
    });
    // Adicionar sugestões para atributos opcionais e valores permitidos
    const optionalAttrsMap = {
    ...
    const allCustomTags = Object.keys(confluenceSchema_1.allowedTags);
        ...
        provideCompletionItems(document, position) {
            ...
            if (line.endsWith('<')) {
                return allCustomTags.map(tag => {
                    const requiredAttrs = confluenceSchema_1.allowedTags[tag] || [];
            ...
            if (tagOpenMatch) {
                ...
                if (allAttrs.length > 0) {
                    return allAttrs.map(attr => {
                        ...
                        if (confluenceSchema_1.allowedValues[allowedKey]) {
                            item.documentation = 'Valores permitidos: ' + confluenceSchema_1.allowedValues[allowedKey].join(', ');
                        }
                        return item;
                    });
                }
            ...
            if (attrValueMatch) {
                ...
                const allowedKey = `${tag}@${attr}`;
                if (confluenceSchema_1.allowedValues[allowedKey]) {
                    return confluenceSchema_1.allowedValues[allowedKey].map((val) => {
    ...
    const diffWithPublishedCmd = vscode.commands.registerCommand('confluence-smart-publisher.diffWithPublished', async (uri) => {
        if (!uri || !uri.fsPath.endsWith('.confluence')) {
        ...
        try {
            await vscode.window.withProgress({
                ...
                const publishedPath = await client.downloadConfluencePage(fileId, confluenceClient_2.BodyFormat.STORAGE, temp);
                // Lê e formata o conteúdo local
                const localContent = fs.readFileSync(uri.fsPath, 'utf-8');
                const formattedLocal = (0, confluenceFormatter_1.formatConfluenceDocument)(localContent, numberChapters, outputChannel);
                ...
                const formattedPublished = (0, confluenceFormatter_1.formatConfluenceDocument)(publishedContent, numberChapters, outputChannel);
    ...
    const syncWithPublishedCmd = vscode.commands.registerCommand('confluence-smart-publisher.syncWithPublished', async (uri) => {
        if (!uri || !uri.fsPath.endsWith('.confluence')) {
            ...
            await vscode.window.withProgress({
                ...
                if (escolha.label === 'Atualizar arquivo local com o conteúdo online') {
                    ...
                    await vscode.window.withProgress({
                        ...
                        await (0, confluenceClient_1.publishConfluenceFile)(uri.fsPath);
    ...
    const setEmojiTitleWebviewCmd = vscode.commands.registerCommand('confluence-smart-publisher.setEmojiTitle', async (uri) => {
        if (!uri || !uri.fsPath.endsWith('.confluence')) {
            vscode.window.showErrorMessage('Selecione um arquivo .confluence para definir o emoji do título.');
            return;
        }
        const panel = vscode.window.createWebviewPanel('cspEmojiPicker', 'Selecione o Emoji do Título', vscode.ViewColumn.Active, {
            enableScripts: true,
        });
    ...
    function getEmojiPickerHtml(webview, extensionUri) {
    ...
    const decodeHtmlCmd = vscode.commands.registerCommand('confluence-smart-publisher.decodeHtml', async (uri) => {
        if (!uri || !uri.fsPath.endsWith('.confluence')) {
